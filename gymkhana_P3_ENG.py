...
################################
### eval seguro
###  importaciones con Restricted Python
...
################################
### fin de imports para Restricted Python
...
def register():
    uo_number = input('Type your UO number (format: UOxxxxx): ')
    uo_number = check_uo(uo_number)
    while (uo_number==''):
        uo_number = input('Wrong UO number format.\nType your UO number (format: UOxxxxx): ')
        uo_number = check_uo(uo_number)
...
def check_result(result, user_file):
    if (result>0):
        ...
        user_file.write('1\n')
        time.sleep(0.85)
        print('Loading next exercise', end='', flush=True)
        time.sleep(0.85)
        for i in range(3):
...
def check_deadline_finished():
    deadline = time.strptime("11/01/2019 22:00", '%d/%m/%Y %H:%M')
...
def main():
    ...
    if (uo_number!='UO0000'):
        input_pass = input('Type your password: ')
