...
def jv_ex7(slvd):
    ...
    msg = """======================================
Teclear  >>> exit    para salir del programa.

Dado el siguiente codigo:
    %s = 34
    %s = %s

Escribe la expresion para evaluar que %s contiene el valor almacenado en %s.
>>>"""%(p[0],p[1],p[2],p[1],p[0]) 
    
    #print(msg)
    
    answer = input(msg) #input(msg)
...
Escribe la expresion que evalua si %s contiene el valor almacenado en %s.
>>>"""%(p[0],p[1],p[2],p[1],p[0]) 
...
Escribe la expresion para incluir la lista %s al final de %s sin crear una nueva lista, de forma que la longitud de %s se incremente en el número de elementos de %s.
...
Escribe la expresión para incluir la lista %s al final de %s sin crear una nueva lista, de manera que la longitud de %s se incrementa en 1.
...
Escribe la expresión que determine si %s esta incluida en $s o no.
>>>"""%(p[0], str(p[2]), p[1], str(p[3]), p[0], p[1]) 
...
Puedes usar o bien la función len o índices negativos para obtener la posición
del último elemento.
Use type(int(-1)) para determinar el tipo de un entero.
>>>"""%(p[0], str(p[1]), p[0]) 
...
Escribe la expresión que evalúa si el primer elemento en $s esta en minúsculas.
DEBES USAR el metodo lower() de las str.
>>>"""%(p[0], str(p[1]), p[0]) 
...
Teclea la expresión para evaluar si el primer elemento en $s esta en minúsculas.
USAR SOLO operadores de comparación (se prohibe el uso de ord() y chr()).
>>>"""%(p[0], str(p[1]), p[0]) 
    
    #print(msg)
    
    answer = input(msg).replace('"',"'") #input(msg).replace('"',"'")
...
Teclea la expresión para evaluar si %s pertenece al intervalo [%s, %s].
USAR SOLO operadores de comparación sin anidamiento (no vale a<b<c<d).
